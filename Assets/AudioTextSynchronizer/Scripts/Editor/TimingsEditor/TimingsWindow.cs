...
namespace AudioTextSynchronizer.Editor.Timings
{
    public class TimingsWindow : EditorWindow
    {
        ...
        private async void DrawTopPanel()
        {
            ...
            EditorGUILayout.LabelField("Max Length", GUILayout.Width(70));
            EditorGUI.BeginChangeCheck();
            maxTextLength = EditorGUILayout.IntSlider(maxTextLength, 0, 1000, GUILayout.Width(200));
            if (EditorGUI.EndChangeCheck())
            {
                EditorPrefs.SetInt("ATS_MaxTextLength", maxTextLength);
            }
            ...
            if (generateTimings)
            {
                var data = await whisperEditorHelper.GetData(Data.Clip, maxTextLength);
                if (data != null)
                {
        ...
        public void Init(bool reInitialize = false, string assetPath = null)
        {
            ...
            maxTextLength = EditorPrefs.GetInt("ATS_MaxTextLength", 0);
