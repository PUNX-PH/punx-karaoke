...
namespace AudioTextSynchronizer.Editor.Whisper
{
    public class WhisperEditorHelper
    {
        ...
        public async Task<PhraseAsset> GetData(AudioClip clip, int maxTextLength)
        {
#if WHISPER_UNITY
            if (whisper == null)
            {
            ...
            try
            {
                result = await whisperHelper.GenerateTimings(clip, allLanguages[LanguageIndex].Value, maxTextLength);
            }
            catch (Exception)
            {
                // ignored
